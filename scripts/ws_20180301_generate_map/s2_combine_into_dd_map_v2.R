...
#################### CREATE GFC TREE COVER MASK IN 2014 AT THRESHOLD
system(sprintf("gdal_calc.py -A %s -B %s --co COMPRESS=LZW --outfile=%s --calc=\"%s\"",
               paste0(dd_dir,"tmp_gfc_2004_gt",gfc_threshold,".tif"),
               paste0(gfc_dir,"gfc_lossyear.tif"),
               paste0(dd_dir,"tmp_gfc_2014_gt",gfc_threshold,".tif"),
               paste0("(A>0)*((B>=15)+(B==0))")
))
...
system(sprintf("gdal_sieve.py -st %s %s %s ",
               mmu,
               paste0(dd_dir,"tmp_gfc_2014_gt",gfc_threshold,".tif"),
               paste0(dd_dir,"tmp_gfc_2014_gt",gfc_threshold,"_sieve.tif")
))
...
system(sprintf("gdal_calc.py -A %s -B %s --co COMPRESS=LZW --outfile=%s --calc=\"%s\"",
               paste0(dd_dir,"tmp_gfc_2014_gt",gfc_threshold,".tif"),
               paste0(dd_dir,"tmp_gfc_2014_gt",gfc_threshold,"_sieve.tif"),
               paste0(dd_dir,"tmp_gfc_2014_gt",gfc_threshold,"_inf.tif"),
               paste0("(A>0)*(A-B)+(A==0)*(B==1)*0")
))

#################### COMBINATION INTO DD MAP (1==NF, 2==F, 3==Df, 4==Dg, 11==agriculture, 12==HCS_f, 13==HSC_df, 14==HSC_dg, 15==Small forest)
system(sprintf("gdal_calc.py -A %s -B %s -C %s -D %s -E %s -F %s --co COMPRESS=LZW --outfile=%s --calc=\"%s\"",
               ...
               paste0("((A==0)*1+(A>0)*(D==0)*((B==0)*(C==0)*((E>0)*2+(F>0)*15)+(B>0)*3+(C>0)*4)+(A>60)*(D>0)*((B==0)*(C==0)*12+(B>0)*13+(C>0)*14)+(D>0)*(A<60)*11)")
))
...
my_classes <- c(0,1,2,3,4,11,12,13,14,15)
my_colors  <- col2rgb(c("black","grey","darkgreen","red","orange","purple","lightgreen","pink","yellow","turquoise"))
